...
fn evaluates_absent_indefinite_and_malformed_persisted_snooze_values() {
    ...
    assert_eq!(evaluate_snooze(Some("indefinite"), NOW), SnoozeState::Indefinite);
    assert_eq!(evaluate_snooze(Some("not-a-valid-timestamp"), NOW), SnoozeState::Malformed);
...
fn evaluates_future_past_and_boundary_timed_snoozes() {
    assert_eq!(
        ...
        SnoozeState::Active { until_iso: "2026-01-01T13:00:00+00:00".to_string() },
    );
    assert_eq!(evaluate_snooze(Some("2026-01-01T11:59:50+00:00"), NOW), SnoozeState::Expired);
