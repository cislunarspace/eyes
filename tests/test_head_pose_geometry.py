...
These tests exercise `rotation_to_yaw_roll` and `transform_to_yaw_roll`
in isolation from any detector model. They use hand-constructed rotation
matrices (identity, pure yaw, pure roll, combined) to verify the sign
convention documented on `HeadPose`.

The helper functions `_yaw_matrix` and `_roll_matrix` are copied from
`test_detector.py` to keep the geometry tests self-contained. The
original `TestEulerFromRotationMatrix` tests in `test_detector.py`
continue to work via the `_euler_from_rotation_matrix` re-export;
these new tests exercise the same math through the public API names.
"""
...
from eyes.head_pose_geometry import rotation_to_yaw_roll, transform_to_yaw_roll
...
def _roll_matrix(degrees: float) -> np.ndarray:
    """Return a 3×3 rotation matrix for roll rotation (about the Z axis)."""
    r = math.radians(degrees)
    c, s = math.cos(r), math.sin(r)
    return np.array([
        [c, -s, 0.0],
...
# ---------------------------------------------------------------------------
# rotation_to_yaw_roll — pure 3×3 rotation tests
# ---------------------------------------------------------------------------


class TestRotationToYawRoll:
    """Sign-convention tests for the 3×3 rotation → yaw/roll helper."""

    def test_identity_gives_zero_yaw_roll(self) -> None:
        pose = rotation_to_yaw_roll(np.eye(3))
        assert abs(pose.yaw) < 0.01
        assert abs(pose.roll) < 0.01

    def test_positive_yaw(self) -> None:
        pose = rotation_to_yaw_roll(_yaw_matrix(12.0))
    ...
    def test_positive_roll(self) -> None:
        pose = rotation_to_yaw_roll(_roll_matrix(10.0))
    ...
    def test_combined_yaw_and_roll(self) -> None:
        ...
        pose = rotation_to_yaw_roll(rotation)
        assert pose.yaw == pytest.approx(12.0, abs=0.2)
        assert pose.roll == pytest.approx(10.0, abs=0.2)

    def test_large_positive_yaw(self) -> None:
        pose = rotation_to_yaw_roll(_yaw_matrix(45.0))
        assert pose.yaw == pytest.approx(45.0, abs=0.5)

    def test_large_negative_roll(self) -> None:
        pose = rotation_to_yaw_roll(_roll_matrix(-45.0))
        assert pose.roll == pytest.approx(-45.0, abs=0.5)
    ...
    def test_rejects_non_3x3_matrix(self) -> None:
        with pytest.raises(ValueError, match="Expected a 3×3"):
            rotation_to_yaw_roll(np.eye(4))

    def test_rejects_2x2_matrix(self) -> None:
        with pytest.raises(ValueError, match="Expected a 3×3"):
            rotation_to_yaw_roll(np.eye(2))


# ---------------------------------------------------------------------------
# transform_to_yaw_roll — 4×4 convenience wrapper
# ---------------------------------------------------------------------------


class TestTransformToYawRoll:
    """Tests for the 4×4 → HeadPose convenience wrapper."""

    def test_4x4_identity_gives_zero_yaw_roll(self) -> None:
        pose = transform_to_yaw_roll(np.eye(4))
        assert abs(pose.yaw) < 0.01
        assert abs(pose.roll) < 0.01
    ...
    def test_4x4_yaw_rotation(self) -> None:
        ...
        pose = transform_to_yaw_roll(transform)
        assert pose.yaw == pytest.approx(20.0, abs=0.1)

    def test_4x4_roll_rotation(self) -> None:
        transform = np.eye(4)
        transform[:3, :3] = _roll_matrix(15.0)
        pose = transform_to_yaw_roll(transform)
        assert pose.roll == pytest.approx(15.0, abs=0.1)
    ...
    def test_flat_16_element_list(self) -> None:
        ...
        pose = transform_to_yaw_roll(flat)
...
class TestDetectorStillExportsLegacyName:
    ...
    def test_legacy_name_same_as_rotation_to_yaw_roll(self) -> None:
        from eyes.detector import _euler_from_rotation_matrix
        from eyes.head_pose_geometry import rotation_to_yaw_roll
        assert _euler_from_rotation_matrix is rotation_to_yaw_roll
